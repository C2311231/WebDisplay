...
class UpdateManager(src.module.module):
    ...
    def __init__(self, device: src.device.Device, repo_url: str = REPO_URL, content_url: str = CONTENT_URL,  local_dir: str = LOCAL_DIR, archive_dir:str = ARCHIVE_DIR, branch: str = BRANCH):
        self.device = device
        device.require_modules("api_registry")
...
def register(device):
    return "updater", UpdateManager(device)
