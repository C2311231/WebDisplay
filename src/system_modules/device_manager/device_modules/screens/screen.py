...
import src.system
import src.system_modules.device_manager.device as device
from src.system_modules.database.setting import Setting
from src.system_modules.database.settings_manager import SettingsManager
import uuid

class Screen():
    def __init__(self, system: src.system.system, device: device.Device, screen_name: str, x: int, y: int, port: str, resolution_x: int, resolution_y: int, volume: float, sound_device: str, screen_type: str, scale: float, screen_id: str = ""):
        self.system = system
        self.device = device
    ...
    @classmethod
    def from_db(cls, system: src.system.system, device: device.Device, screen_id: str):
        """
        Generates an existing screen from the db

        Args:
            system (src.system.system): _description_
